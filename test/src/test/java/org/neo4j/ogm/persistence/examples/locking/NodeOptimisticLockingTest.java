...
public class NodeOptimisticLockingTest extends MultiDriverTestClass {
     ...
     *
     Collection<User> users = session.loadAll(User.class);
     assertThat(users).containsOnly(frantisek);

     User loaded = users.iterator().next();
     assertThat(loaded.getVersion()).isEqualTo(0L);
     */

    // save relationship entity

    // fail on relationship entity

    // failed entity can be reloaded on same session (node & relationship)

    //

    // should fail default transactions
